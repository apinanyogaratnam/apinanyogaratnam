...
file_contents = {}
for file in get_all_files_in_current_directory_recursively(os.getcwd(), list_of_files_to_ignore):
    ...
    for file in get_all_files_in_current_directory_recursively(os.getcwd(), list_of_files_to_ignore):
        with open(file, 'r') as f:
            ...
            if file_contents.get(file) != current_content:
                ...
                index_of_lastmod_end = sitemap_contents.find('</lastmod>', index_of_lastmod)
                if index_of_lastmod != -1 and index_of_lastmod_end != -1:
                    sitemap_contents = sitemap_contents[:index_of_lastmod + 9] + datetime.now().strftime('%Y-%m-%d') + sitemap_contents[index_of_lastmod_end:]
